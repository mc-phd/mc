                 ...
                 const vfs_path_t *dst_vpath, gboolean *skip_all)
{
    ...
    if (lnk != NULL)
    {
            ...
            if (lp_name_class == my_vfs)
            {
                ...
                if (dst_name_class == p_class)
                {
                    ...
                    while (!(ok = (mc_link (lnk->dst_vpath, dst_vpath) == 0)) && !*skip_all)
                    {
                        ...
                        status = file_error (TRUE, _ ("Cannot create target hardlink \"%s\"\n%s"),
        ...
        if (!*skip_all)
        {
             ...
             */
            errno = 0;
            status = file_error (FALSE, _ ("Cannot create target hardlink \"%s\""),
                                 vfs_path_as_str (dst_vpath));
...
real_do_file_error (enum OperationMode mode, gboolean allow_retry, const char *error)
{
    ...
    if (allow_retry)
        result = query_dialog (msg, error, D_ERROR, 4, _ ("&Skip"), _ ("Ski&p all"), _ ("&Retry"),
                               _ ("&Abort"));
    else
...
FileProgressStatus
copy_file_file (file_op_total_context_t *tctx, file_op_context_t *ctx, const char *src_path,
                const char *dst_path)
{
    ...
    while (attrs_ok && mc_fgetflags (src_vpath, &attrs) != 0)
    {
        ...
        {
            return_status = file_error (TRUE, _ ("Cannot get attributes of source file \"%s\"\n%s"),
                                        src_path);
            if (return_status == FILE_SKIPALL)
                ctx->skip_all = TRUE;
            if (return_status == FILE_ABORT)
    ...
    if (!ctx->do_append)
    {
                ...
                while (attrs_ok && mc_fsetflags (dst_vpath, attrs) != 0 && !ctx->skip_all)
                {
                    ...
                    if (errno == ENOTSUP)
                        ...
                        return_status = file_error (
                            TRUE, _ ("Cannot set attributes of target file \"%s\"\n%s"), dst_path);
            ...
            || S_ISNAM (src_stat.st_mode) || S_ISSOCK (src_stat.st_mode))
        {
            ...
            while (mc_mknod (dst_vpath, src_stat.st_mode & ctx->umask_kill, rdev) < 0
                   && !ctx->skip_all)
            {
                return_status = file_error (TRUE, _ ("Cannot create special file \"%s\"\n%s"),
                                            dst_path);
                if (return_status == FILE_RETRY)
                    continue;
                if (return_status == FILE_SKIPALL)
    ...
    while ((dest_desc = mc_open (dst_vpath, open_flags, src_mode)) < 0)
    {
        if (errno != EEXIST)
        {
            ...
            {
                return_status = file_error (TRUE, _ ("Cannot create target file \"%s\"\n%s"),
                                            dst_path);
                if (return_status == FILE_RETRY)
                    continue;
                if (return_status == FILE_SKIPALL)
           ...
           && vfs_preallocate (dest_desc, file_size, appending ? dst_stat.st_size : 0) != 0)
    {
        ...
        return_status = file_error (TRUE, _ ("Cannot preallocate space for target file \"%s\"\n%s"),
                                    dst_path);
    ...
    if (return_status == FILE_CONT)
    {
        ...
        while (TRUE)
        {
            ...
            if (mc_ctl (src_desc, VFS_CTL_IS_NOTREADY, 0) == 0)
                ...
                {
                    return_status = file_error (TRUE, _ ("Cannot read source file \"%s\"\n%s"),
                                                src_path);
                    if (return_status == FILE_RETRY)
                        continue;
                    if (return_status == FILE_SKIPALL)
            ...
            if (n_read > 0)
            {
                ...
                while ((n_written = mc_write (dest_desc, t, (size_t) n_read)) < n_read)
                {
                    ...
                    if (ctx->skip_all)
                        ...
                        return_status = file_error (TRUE, _ ("Cannot write target file \"%s\"\n%s"),
                                                    dst_path);
            ...
            if (return_status != FILE_CONT)
            {
                ...
                query_res = query_dialog (Q_ ("DialogTitle|Copy"),
...
retry_src_stat:
    ...
    while (attrs_ok && mc_fgetflags (src_vpath, &attrs) != 0)
    {
        ...
        {
            return_status = file_error (
                TRUE, _ ("Cannot get attributes of source directory \"%s\"\n%s"), s);
            if (return_status == FILE_RETRY)
            {
                attrs_ok = TRUE;
                continue;
...
retry_dst_stat:
    ...
    else
    {
        ...
        if (!S_ISDIR (dst_stat.st_mode))
        {
            ...
            {
                return_status = file_error (TRUE, _ ("Destination \"%s\" must be a directory\n%s"),
                                            d);
                if (return_status == FILE_SKIPALL)
                    ctx->skip_all = TRUE;
                if (return_status == FILE_RETRY)
    ...
    if (do_mkdir)
    {
        while (my_mkdir (dst_vpath, (src_stat.st_mode & ctx->umask_kill) | S_IRWXU) != 0)
        {
            ...
            {
                return_status = file_error (TRUE, _ ("Cannot create target directory \"%s\"\n%s"),
                                            d);
                if (return_status == FILE_SKIPALL)
                    ctx->skip_all = TRUE;
            }
    ...
    while ((next = mc_readdir (reading)) && return_status != FILE_ABORT)
    {
        ...
        if (S_ISDIR (dst_stat.st_mode))
        {
             ...
             * meaning no toplevel.
             */
            return_status = copy_dir_dir (tctx, ctx, path, mdpath, FALSE, FALSE, do_delete,
                                          parent_dirs);
            g_free (mdpath);
        }
        else
