...
check_file_access (WEdit *edit, const vfs_path_t *filename_vpath, struct stat *st)
{
    ...
    {
        errmsg = g_strdup_printf (_ ("\"%s\" is not a regular file"),
                                  vfs_path_as_str (filename_vpath));
        goto cleanup;
    }
...
check_bottom:
    ...
    /*If a single key produced enough pushes to wrap all the way round then we would notice that the
     * [undo_stack_bottom] does not contain KEY_PRESS. The stack is then initialised: */
